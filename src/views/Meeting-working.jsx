...
export default function Meeting() {
  ...
  const handleCameraCapture = (imageData) => {
    // Add the captured image as a note
    const note = {
      id: Date.now().toString(),
    ...
    setMeeting(updatedMeeting)
    updateMeeting(updatedMeeting)
    console.log('Photo added to meeting notes')
  }
  ...
  const handleFileUpload = (event) => {
    ...
    files.forEach(file => {
      ...
      // Create file reader for preview/content
      ...
      reader.onload = (e) => {
        ...
        const updatedMeeting = {
          ...meeting,
          notes: [...(meeting.notes || []), note],
          attachments: [...(meeting.attachments || []), fileData]
        }
      ...
      // Read file as data URL for images, or as text for text files
      if (file.type.startsWith('image/')) {
        reader.readAsDataURL(file)
      } else if (file.type.startsWith('text/') || file.type === 'application/json') {
        reader.readAsText(file)
      } else {
        reader.readAsDataURL(file) // For other files, store as data URL
      }
    })
  ...
  const triggerFileUpload = () => {
    ...
    fileInput.accept = 'image/*,text/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip,.rar' // Common file types
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
              <button
                onClick={() => navigate('/')}
                className="flex items-center gap-2 px-3 sm:px-4 py-2 text-gray-600 hover:text-gray-900 rounded-lg hover:bg-gray-100 transition-colors flex-shrink-0"
              >
                <ArrowLeft size={18} />
              ...
              <div className="border-l border-gray-300 pl-2 sm:pl-4 min-w-0 flex-1">
                {isEditing ? (
                  <input
                    ...
                    className="text-lg sm:text-xl font-semibold text-gray-900 bg-transparent border-none focus:outline-none focus:ring-2 focus:ring-blue-500 rounded px-2 py-1 w-full"
                  />
                ) : (
              ...
              {isEditing ? (
                <>
                  <button
                    ...
                    className="px-3 sm:px-4 py-2 text-gray-600 hover:text-gray-900 rounded-lg hover:bg-gray-100 transition-colors whitespace-nowrap"
                  >
                  ...
                  <button
                    onClick={handleSave}
                    disabled={isSaving}
                    className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 whitespace-nowrap"
                  >
                    {isSaving ? (
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors whitespace-nowrap"
                >
                  <Edit size={16} />
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="space-y-3">
                  {uploadedFiles.map((file) => (
                    <div key={file.id} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                      <Upload size={16} className="text-gray-500 mt-1 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 mt-1">
                          ...
                          <p className="text-xs text-gray-500">
                            {format(new Date(file.timestamp), 'MMM d, h:mm a')}
                          </p>
                        </div>
                        {file.type.startsWith('image/') && (
                          <img
                            src={file.content}
                            alt={file.name}
                            className="mt-2 max-w-48 max-h-24 object-cover rounded border"
                          />
                        )}
                      </div>
                      <button
